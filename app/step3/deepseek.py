...
def load_env():
    ...
    if not api_key:
        raise ValueError("DEEPSEEK_API_KEY não encontrado no .env")
    return api_key
...
def call_deepseek(client: OpenAI, prompt: str, system_prompt: str, temperature: float = TEMPERATURE, max_tokens: int = 180) -> str:
...
#     return call_deepseek(client, prompt, system_prompt, temperature=TEMPERATURE, max_tokens=180)
...
```"""
    ...
    content = call_deepseek(client, prompt, system_prompt, temperature=TEMPERATURE, max_tokens=120)
...
def main():
    ...
    tempo_inicial = time.time() # Marca o tempo de início
    ...
    # for idx, table_meta in enumerate(metadata[311:651], 1):
    ...
    tempo_final = time.time() # Marca o tempo de fim
    duracao = tempo_final - tempo_inicial
    # print(f"Tempo com time.time(): {duracao:.4f} segundos")
